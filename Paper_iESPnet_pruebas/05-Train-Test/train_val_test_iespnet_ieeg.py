...
def main():
    ...
    outputfile = save_models + 'model_' + str(experiments)
    avg_train_losses, train_accs, avg_valid_losses, valid_accs = train_model_iespnet(
                                                                                model,                                                                                     
                                                                                hparams, 
                                                                                epochs, 
                                                                                train_data, 
    ...
    print('in training')
    # in training
    outputs_train = test_model_iespnet(model, hparams, best_path, train_data, experiments)
    prediction_tr = get_performance_indices(outputs_train['y_true'], outputs_train['y_prob'], best_thr)
    print()
    print('in validation')        
    # in validation
    outputs_vali  = test_model_iespnet(model, hparams, best_path, vali_data, experiments)
    prediction_va = get_performance_indices(outputs_vali['y_true'], outputs_vali['y_prob'], best_thr)

    print()
    print('in testing')
    # in testing
    outputs_test  = test_model_iespnet(model, hparams, best_path, test_data, experiments)
