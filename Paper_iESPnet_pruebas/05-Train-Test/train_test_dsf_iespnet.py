...
from TrainEval         import train_model_optv2, test_modelv2, get_performance_indices
...
def main():
    ...
    weights = make_weights_for_balanced_classes(train_df, [0,1], n_concat=1) # cambie el n_concat de 2 a 1
    sampler = torch.utils.data.sampler.WeightedRandomSampler(weights, len(weights))

    outputfile = save_models + 'model'
    avg_train_losses, avg_train_f1 = train_model_optv2(model1, model2, hparams, epochs, train_data, transform_train, sampler, outputfile)

    best_thr = 0.2
    best_path = save_models + 'model_opt.pth'

    # in testing
    outputs_test  = test_modelv2(model1, model2, hparams, best_path, test_data)
    prediction_te = get_performance_indices(outputs_test['y_true'], outputs_test['y_prob'], best_thr)
                
    # in training
    outputs_train = test_modelv2(model1, model2, hparams, best_path, train_data)
