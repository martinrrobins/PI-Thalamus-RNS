...
import pandas as pd
import sys
sys.path.insert(1, r'Y:\iESPnet\iESPnet_SRC\utilities')
...
DATA_DIR = r'Y:\RNS_DataBank\PITT'
OUTDATA_DIR = "/mnt/Nexus2/ESPnet/Data/RNS_Databank_Spectrograms/ZeropadAll/PITT/"
OUTMETADATA_DIR = "/mnt/Nexus2/ESPnet/Data/RNS_Databank_Spectrograms/ZeropadAll/PITT/METADATA/"
...
# this are the database used for the ESPdetect
...
for s in range(len(RNSIDS)):
    ...
    for nfile in range(len(data_files)):
        ...
        X, labels, times = Epochsv2.get_epochs_zeropad_all(data_files[nfile], events)
        [nt, nc, ns] = np.shape(X)
        ...
        for e in range(nt):
            ...
            file_name = hosp_id + '_' + subject_id + '_' + PE_id + '_E' + str(e)
            ...
            np.save(OUTDATA_DIR + file_name, spec)
