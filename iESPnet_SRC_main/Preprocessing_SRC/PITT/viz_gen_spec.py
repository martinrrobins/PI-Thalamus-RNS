...
overlap = 500 #
SPEC_WIN_LEN = int(ECOG_SAMPLE_RATE * 1000 / 1000 ) # win size
SPEC_HOP_LEN = int(ECOG_SAMPLE_RATE * (TT - overlap) / 1000)   # Length of hop between windows.
SPEC_NFFT = 500   # Size of FFT, creates ``n_fft // 2 + 1`` bins
#%%
...
top_db = 60.0
#%%
out_address = '/mnt/Nexus2/SeizeConvNet/Preprocessing_plots/'
...
for s in range(len(RNSIDS)):
    ...
    for nfile in range(len(data_files)):
        ...
        X, labels, times = Epochs.get_epochs(data_files[nfile], events)
        ...
        plt.subplot(211)
        plt.plot(tv,epoch)
        plt.vlines(times[eof[0]], min(epoch), max(epoch), color='k')
        plt.xlabel('Time')
        plt.ylabel('Amplitude')
        plt.title(data_files[nfile][52:72] +' NO_SZ ' + ' epoch' + str(0))
        # plot spec
        plt.subplot(212)
        plt.imshow(t, spec, origin='lower', aspect='auto')
        ...
        for l in range(len(szon)):
            ...
            plt.subplot(211)
            plt.plot(tv,epoch)
            plt.vlines(times[szon[l]], min(epoch), max(epoch), color='k')
            plt.xlabel('Time')
            plt.ylabel('Amplitude')
